...
(select 	pid.identifier as nid,
		...
		coorte3meses_final.data_inicio,
		regime.ultimo_regime,
		ultimacv.data_carga,
		ultimacv.valor_carga,
		coorte3meses_final.data_fila,
		coorte3meses_final.data_proximo_lev,
		coorte3meses_final.data_seguimento,
		coorte3meses_final.data_proximo_seguimento,
		coorte3meses_final.patient_id
from
-- 1. Aqui determina-se a data que será usada para ser comparada com a data em que o paciente completa 3 meses de tarv
...
-- 2.1 Se o paciente tiver um estado preenchido (Transferido para, suspenso, abandono, obito) antes dos 3 meses de tarv, este será usado como estado final
-- 2.2 Se o paciente não tiver nenhuma data no ponto 1 (paciente sem nenhum levantamento ou consulta clinica dentro de 3 meses de inicio de TARV) será considerado ABANDONO
-- 2.2 Se a data em que completa 3 meses for maior que a data encontrada em 1 será considerado abandono caso não será activo 
(select   coorte3meses.*,	
...
from
-- Aqui construimos uma tabela que tem os campos: data de inicio de tarv do paciente, data que completa 3 meses, data do ultimo estado antes dos 3 meses,
-- data do ultimo levantamento antes dos 3 meses, data da ultima consulta antes dos 3 meses
-- o estado antes do 3 meses tambem está incluido
(select inicio.*,
...
-- A partir da data de inicio de TARV é adicionado 3 meses.
-- Ex: se data de inicio for 01.01.2016, adicionar 1 ano o MySQL tira 01.01.2017 menos 1 dia a real data sera 31.12.2016
-- A tabela interna 'inicio' é composta por 3 campos: patient_id, data_inicio, data3meses
(	Select patient_id,min(data_inicio) data_inicio,date_add(min(data_inicio), interval 3 MONTH) data3meses
		from
			(	
...
-- Aqui encontramos os estados paciente até antes dos 3 meses, repare que este estado pode não ser o estado actual
left join
...
-- Aqui encontramos os levantamentos de ARV efectuados do paciente antes dos 3 meses de TARV
left join
...
-- Aqui encontramos as consultas efectuadas do paciente antes dos 3 meses de TARV
left join
...
-- Aqui encontramos a data do proximo levantamento marcado no ultimo levantamento de ARV antes dos 3 meses
left join
...
-- Aqui encontramos a data da proxima consulta marcada na ultima consulta antes dos 3 meses
left join
...
inner join person pe on pe.person_id=coorte3meses_final.patient_id and pe.gender='F'
inner join
(
	Select 	p.patient_id
	from 	patient p 
...
) gravida on gravida.patient_id=coorte3meses_final.patient_id
left join 
...
)ultimacv on coorte3meses_final.patient_id=ultimacv.patient_id
left join 
(
...
) regime on regime.patient_id=coorte3meses_final.patient_id
left join 
(	select pid1.*
...
) pid on pid.patient_id=coorte3meses_final.patient_id
left join 
(	select pn1.*
...
) pn on pn.person_id=coorte3meses_final.patient_id
left join person_attribute pa on pa.person_id=coorte3meses_final.patient_id and pa.person_attribute_type_id=9
...
left join 
		(	select patient_id,o.value_text
			...
			group by patient_id
		) pessoa_referencia on pessoa_referencia.patient_id=coorte3meses_final.patient_id
left join 
			(	select pad1.*
			...
			) pad3 on pad3.person_id=coorte3meses_final.patient_id	
left join
		(	select 	p.patient_id,
					case o.value_coded
		...
		)proveniencia on proveniencia.patient_id=coorte3meses_final.patient_id



-- Verificação qual é o estado final
where 	estado_final=6 and 
		coorte3meses_final.patient_id not in 
		(
			Select 	p.patient_id
			from 	patient p inner join encounter e on p.patient_id=e.patient_id
					inner join obs o on e.encounter_id=o.encounter_id
		...
		round(datediff(:endDate,coorte3meses_final.data_inicio)/30)>=3
) elegiveiscv
group by patient_id